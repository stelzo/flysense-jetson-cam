...
namespace npp
{
    ...
    */
                             ...
                             size_t *datalength, size_t maxlength, cudaStream_t stream)
    {
        ...
        {
            char info[256];
            sprintf(info, "FATAL ERROR: Pre-malloced jpeg data size is too small ! nScanLength = %d, maxlength = %d", (int)nScanLength, (int)maxlength);
    ...
    */
                             ...
                             size_t *datalength, size_t maxlength, cv::cuda::Stream &cvstream)
    {
        ...
        NPP_CHECK_NPP(nppiCFAToRGB_8u_C1C3R(bayer_img_d.data, bayer_img_d.step, osize, orect, rgb_img_d, step_rgb, cfaBayerType, NPPI_INTER_UNDEFINED));
#else
        // cv::cuda::demosaicing(bayer_img_d, rgb_img_mat_d, npp::bayerPatternNPP2CVRGB(cfaBayerType), -1, std::ref(cvstream));
#endif
        // cudaStreamSynchronize(stream);
        //  cv::Mat img;
        //  bayer_img_d.download(img);
        rgb_img_d = (Npp8u *)bayer_img_d.cudaPtr();
        // rgb_img_mat_d = bayer_img_d;
